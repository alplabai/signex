...
pub fn write_schematic(sheet: &SchematicSheet) -> String {
    ...
    for sym in &sheet.symbols {
        ...
        writeln!(out, "\t\t(lib_id \"{}\")", escape_kicad_string(&sym.lib_id)).unwrap();
        writeln!(
            out,
            "\t\t(at {} {} {})",
        ...
        .unwrap();
        if sym.mirror_x {
            writeln!(out, "\t\t(mirror x)").unwrap();
        }
        if sym.mirror_y {
            writeln!(out, "\t\t(mirror y)").unwrap();
        }
    ...
    for ne in &sheet.no_erc_directives {
        writeln!(out, "\t(no_erc").unwrap();
        writeln!(out, "\t\t(at {} {})", ne.position.x, ne.position.y).unwrap();
        writeln!(out, "\t\t(uuid \"{}\")", ne.uuid).unwrap();
        writeln!(out, "\t)").unwrap();
    }
...
fn write_property(out: &mut String, key: &str, value: &str, text: &TextProp, sym_rot: f64) {
    ...
    if text.justify_h != "center" || text.justify_v != "center" {
        ...
        if text.justify_h != "center" {
            write!(out, " {}", text.justify_h).unwrap();
        }
        if text.justify_v != "center" {
            write!(out, " {}", text.justify_v).unwrap();
        }
        write!(out, ")").unwrap();
    }
    writeln!(out, ")").unwrap();
...
fn write_lib_symbol(out: &mut String, _id: &str, lib: &LibSymbol) {
    ...
    writeln!(out, "\t\t(symbol \"{}\"", escape_kicad_string(&lib.id)).unwrap();
