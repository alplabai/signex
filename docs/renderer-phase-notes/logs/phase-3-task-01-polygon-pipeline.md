...
- Pending: extend validation for non-convex or pre-clipped production polygon inputs.
...
- Verification: `cargo test -p signex-gfx -- --nocapture` and `cargo check -p signex-gfx -p signex-renderer` passed.

## Artifacts

- PR/commit: local workspace changes, commit pending
- Test output: signex-gfx tests passed (7 passed, 0 failed)
