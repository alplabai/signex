...
// ---------------------------------------------------------------------------
// Enum-to-Standard-string helpers
// ---------------------------------------------------------------------------
...
fn pin_electrical_str(t: PinDirection) -> &'static str {
    match t {
        ...
        // additions or unmapped variants. Standard's narrower vocabulary lacks
...
fn pin_shape_str(s: PinShapeStyle) -> &'static str {
    match s {
        ...
        // Hysteresis* / Schmitt have no Standard equivalent — emit plain "line"
...
/// Standard writes alpha as a 0..1 float in `(color ...)` quads; convert
...
/// `(fill ...)` for `(sheet ...)` blocks. Standard serializes the sheet body
...
fn label_node(l: &Label) -> SExpr {
    ...
    // left-horizontal + bottom-vertical case so round-tripping a Standard
...
fn standard_sch_root_node(sheet: &SchematicSheet) -> SExpr {
    let mut items = vec![
...
/// Serialize a [`SchematicSheet`] to the Standard `.standard_sch` S-expression format.
pub fn write_schematic(sheet: &SchematicSheet) -> String {
    let mut out = String::with_capacity(64 * 1024);
    write_rendered_sexpr(&mut out, 0, standard_sch_root_node(sheet));
    out
}
...
mod tests {
    ...
    #[test]
    fn writes_property_metadata_in_standard_order() {
        let text = TextProp {
    ...
    fn writes_custom_property_with_ast_metadata() {
        ...
        let parsed = standard_parser::sexpr::parse(out.trim()).unwrap();
    ...
    fn writes_sheet_instances_root_page_and_symbol_instances() {
        ...
        let parsed = standard_parser::sexpr::parse(&rendered).unwrap();
    ...
    fn writes_child_sheet_with_instances_as_expected_sexpr() {
        let child = ChildSheet {
            ...
            filename: "power.standard_sch".to_string(),
            position: Point { x: 10.0, y: 20.0 },
            size: (30.0, 40.0),
            stroke_width: 0.12,
            fill: FillType::None,
