...
fn write_symbol(out: &mut String, sym: &Symbol) {
    ...
    // Footprint and Datasheet: always hidden at property level (Standard 8 format).
    wln!(
        out,
        "    (property \"Footprint\" \"{}\"",
...
fn write_property(out: &mut String, key: &str, value: &str, text: &TextProp, _sym_rot: f64) {
    // prop.rotation is stored in world-frame (same as Standard file storage).
    ...
    wln!(out, "      (show_name no)");
    wln!(out, "      (do_not_autoplace no)");
    // Standard 8: (hide yes) is a direct child of the property node, NOT inside
