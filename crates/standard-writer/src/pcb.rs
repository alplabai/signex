...
/// Serialize a [`PcbBoard`] to the Standard `.standard_pcb` S-expression format.
pub fn write_pcb(board: &PcbBoard) -> String {
    let mut out = String::with_capacity(64 * 1024);

    wln!(out, "(standard_pcb");
...
mod tests {
    ...
    fn assert_fragment_matches(actual: &str, expected: SExpr) {
        let parsed = standard_parser::sexpr::parse(actual).unwrap();
    ...
    fn writes_structured_footprint_properties_without_duplicate_text_graphics() {
        ...
        let parsed = standard_parser::sexpr::parse(&out).unwrap();
        let property = parsed
            .find_all("property")
            .into_iter()
