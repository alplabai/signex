...
///
/// `display_pos`: the Standard `GetPosition()` display position — the stored field
...
pub fn draw_text_prop(
    ...
    // Standard SCH_FIELD::GetEffectiveJustify(): when symbol transform.x1 < 0
    ...
    // Standard angle: 0 = horizontal, 90 = vertical (reads bottom-to-top = CCW).
