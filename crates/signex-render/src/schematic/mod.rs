...
/// Compute the display position of a symbol field, equivalent to Standard's
/// `SCH_FIELD::GetPosition()`.
///
/// Standard stores field positions as absolute schematic coordinates, but the
...
pub(super) fn field_display_pos(
    ...
    // Mirrors applied after rotation (Standard convention).
