...
fn instance_transform(sym: &Symbol, local: &Point) -> (f64, f64) {
    let lx = local.x;
    // Standard library coords are Y-up (math), schematic coords are Y-down (screen).
    // Flip Y at the boundary.
    let ly = -local.y;

    // Mirror (applied before rotation, Standard convention)
    ...
    // with Standard's stored rotation angle.
