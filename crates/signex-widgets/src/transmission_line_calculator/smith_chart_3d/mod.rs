...
use super::smith_view_navigation::SmithViewNavigationState;
use projected_point::ProjectedPoint;
use smith_chart_3d_canvas_state::SmithChart3dCanvasState;
use sphere_stroke::SphereStroke;

#[cfg(test)]
#[path = "../../../tests/transmission_line_calculator/smith_chart_3d_tests.rs"]
mod tests;
...
impl Default for SmithChart3dCanvasState {
    ...
    fn default() -> Self {
        Self {
            ...
            drag_yaw: 0.0,
            drag_pitch: 0.0,
            navigation: SmithViewNavigationState::default(),
        }
    }
}
...
impl canvas::Program<SmithChartMessage> for SmithChart3dCanvas {
    ...
    fn update(
        ...
        if let Some(action) = state
            .navigation
            .update(event, bounds, cursor, self.view_transform)
        {
            return Some(action);
        }
    ...
    fn draw(
        ...
        let geometry = canvas::Cache::new().draw(renderer, bounds.size(), |frame| {
            ...
            let center = self
                .view_transform
                .transform_point(Point::new(bounds.width / 2.0, bounds.height / 2.0));
            let radius = bounds.width.min(bounds.height) * 0.39 * self.view_transform.scale();
            ...
            draw_label(
                frame,
                "Left-drag to rotate • Wheel to zoom • Middle-drag to pan".to_string(),
    ...
    fn mouse_interaction(
        ...
        if state.drag_start.is_some() || state.navigation.is_panning() {
...
/// Adds sphere outline to the current aggregate.
...
/// Adds smith grid to the current aggregate.
fn add_smith_grid(
    strokes: &mut Vec<SphereStroke>,
...
/// Adds complex traces to the current aggregate.
...
/// Adds latitude to the current aggregate.
fn add_latitude(
    strokes: &mut Vec<SphereStroke>,
    z: f32,
    yaw: f32,
...
/// Adds meridian to the current aggregate.
fn add_meridian(
    strokes: &mut Vec<SphereStroke>,
    longitude: f32,
    yaw: f32,
...
/// Adds sphere polyline to the current aggregate.
...
/// Draws stroke into the target drawing surface.
...
/// Draws sphere dot into the target drawing surface.
fn draw_sphere_dot(
...
/// Projects a rotated Smith-sphere point into screen coordinates.
fn project(
    point: SmithSpherePoint,
    yaw: f32,
    pitch: f32,
