...
fn starting_impedance(
    ...
    match circuit.first() {
        ...
        Some(SmithChartElement::SParameter(block)) => s_parameter_point_at(block, frequency_hz)
...
fn solve_s_parameter_gain_for_sides(
    ...
    let mut out = Vec::new();
    for frequency_hz in frequencies.iter().copied() {
        let Some(point) = s_parameter_point_at(block, frequency_hz) else {
            continue;
        };
        ...
        .map(|result| result.impedance)
        .unwrap_or(Complex::new(settings.reference_impedance_ohm, 0.0));
        let gamma_source = impedance_to_reflection(source_z, block.reference_impedance_ohm);
        let gamma_load = impedance_to_reflection(load_z, block.reference_impedance_ohm);
        let s11 = point.s11;
        let s22 = point.s22.unwrap_or(Complex::ZERO);
        let s21 = point.s21.unwrap_or(Complex::ZERO);
        out.push(GainPoint {
            frequency_hz,
            transducer_gain_linear: mismatch_gain(gamma_source, s11)
                * mismatch_gain(gamma_load, s22)
                * s21.magnitude().powi(2),
        });
    }
    Ok(out)
}
...
pub fn solve_s_parameter_gain_circles(
    ...
    let Some(point) = s_parameter_point_at(block, active_frequency_hz) else {
        return Vec::new();
    };
    let mut out = Vec::new();
...
fn s_parameter_point_at(block: &SParameterBlock, frequency_hz: f64) -> Option<&SParameterPoint> {
    block
        .points
        .iter()
        .find(|point| point.frequency_hz >= frequency_hz)
        .or_else(|| block.points.last())
}
...
fn mismatch_gain(gamma: Complex, s_gamma: Complex) -> f64 {
    (1.0 - gamma.magnitude().powi(2))
        / ((1.0 - s_gamma.re * gamma.re + s_gamma.im * gamma.im).powi(2)
            + (s_gamma.im * gamma.re + s_gamma.re * gamma.im).powi(2))
}
