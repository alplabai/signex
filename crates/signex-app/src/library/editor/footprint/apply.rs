//! Footprint-editor message reducers, extracted from the library
//! dispatcher (issue #98 — decomposing the 10k-line
//! `app/dispatch/library.rs`). Pure state reduction over a
//! `FootprintEditorState`; the reducer's per-arm helper closures and
//! `use` imports are arm-local, so the functions move verbatim with no
//! behavioural change. External sketch/pad helpers are already `pub` in
//! sibling modules (`sketch_dispatch`, `pad_to_sketch`).
...
pub(crate) fn apply_footprint_primitive_edit(
    ...
    match msg {
        ...
        PrimitiveEditorMsg::FootprintSketchMovePoint { id, dx, dy } => {
            apply_footprint_sketch_move_point(editor, id, dx, dy);
        }
        PrimitiveEditorMsg::FootprintSketchMoveLine { id, dx, dy } => {
            apply_footprint_sketch_move_line(editor, id, dx, dy);
        }
        ...
        PrimitiveEditorMsg::FootprintSketchSetRole { id, role } => {
            apply_footprint_sketch_set_role(editor, id, role);
        }
        PrimitiveEditorMsg::FootprintSketchMakePadFromProfile => {
            apply_footprint_sketch_make_pad_from_profile(editor);
        }
        PrimitiveEditorMsg::FootprintSketchUnlinkCornerRadius { arc_entity_id } => {
            apply_footprint_sketch_unlink_corner_radius(editor, arc_entity_id);
        }
        PrimitiveEditorMsg::FootprintSketchAddConstraintForSelection(tag) => {
            apply_footprint_sketch_add_constraint_for_selection(editor, tag);
        }
        PrimitiveEditorMsg::FootprintSketchToolClick {
            ...
            apply_footprint_sketch_tool_click(editor, x_mm, y_mm, snap_id);
        }
...
/// Handle a click in a footprint sketch-mode tool (the largest arm of
/// `apply_footprint_primitive_edit`, extracted for readability — issue
/// #98). Behaviour is unchanged; the body moved verbatim.
fn apply_footprint_sketch_tool_click(
    editor: &mut crate::app::FootprintEditorState,
    x_mm: f64,
    y_mm: f64,
...
/// Convert the closed-loop profile containing the selected line into a
/// pad (extracted arm body — issue #98; behaviour unchanged).
fn apply_footprint_sketch_make_pad_from_profile(editor: &mut crate::app::FootprintEditorState) {
...
/// Drag a sketch Line edge by translating both its endpoints (extracted
/// arm body — issue #98; behaviour unchanged).
fn apply_footprint_sketch_move_line(
    editor: &mut crate::app::FootprintEditorState,
...
/// Drag a single sketch Point, updating dependent geometry (extracted
/// arm body — issue #98; behaviour unchanged).
fn apply_footprint_sketch_move_point(
    editor: &mut crate::app::FootprintEditorState,
    id: signex_sketch::id::SketchEntityId,
    dx: f64,
    dy: f64,
...
/// Add a sketch constraint for the current selection (extracted arm — #98).
fn apply_footprint_sketch_add_constraint_for_selection(
    editor: &mut crate::app::FootprintEditorState,
...
/// Unlink a RoundRect corner's shared radius parameter (extracted arm — #98).
fn apply_footprint_sketch_unlink_corner_radius(
    editor: &mut crate::app::FootprintEditorState,
...
/// Set the role attribute on a sketch entity (extracted arm — #98).
fn apply_footprint_sketch_set_role(
    editor: &mut crate::app::FootprintEditorState,
    id: signex_sketch::id::SketchEntityId,
