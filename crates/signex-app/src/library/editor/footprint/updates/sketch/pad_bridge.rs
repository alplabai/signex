...
pub(in crate::library::editor::footprint::updates) fn apply(
    ...
    match msg {
        FootprintEditorMsg::SketchSetRole { id, role } => set_role(editor, id, role),
        FootprintEditorMsg::SketchMakePadFromProfile => make_pad_from_profile(editor),
        FootprintEditorMsg::SketchUnlinkCornerRadius { arc_entity_id } => {
            unlink_corner_radius(editor, arc_entity_id)
        }
        _ => unreachable!(
    ...
    let vertices = match trace {
        ...
        Ok(_) => {
            editor
                .state
                .solve_warnings
                .push("Make Pad from Profile: traced loop has fewer than 3 vertices".into());
            editor.canvas_cache.clear();
            return;
        }
        Err(e) => {
