...
pub(super) fn apply(
    ...
    match tool {
        SketchTool::Fillet => fillet(editor, ctx),
        SketchTool::Trim => trim(editor, ctx),
        SketchTool::BreakTrack => break_track(editor, ctx),
...
fn fillet(editor: &mut crate::app::FootprintEditorState, ctx: &ToolClickCtx) {
    ...
    match editor.state.tool_pending {
        ToolPending::FilletFirst { line: first_line } => {
            fillet_second_click(editor, ctx, click_xy, first_line, radius_mm)
        }
        _ => fillet_first_click(editor, click_xy),
    }
}
...
fn fillet_second_click(
    ...
    if cap < radius_mm * 0.05 {
        editor
            .state
            .solve_warnings
            .push("Fillet: radius too large for these lines — pick a smaller r".into());
        editor.state.tool_pending = ToolPending::Idle;
        return;
    }
...
fn trim(editor: &mut crate::app::FootprintEditorState, ctx: &ToolClickCtx) {
    ...
    let Some(target_line) = target_line else {
        editor
            .state
            .solve_warnings
            .push("Trim: click missed any Line — try clicking closer to a line stroke".into());
        editor.state.tool_pending = ToolPending::Idle;
        return;
    };
...
fn break_track(editor: &mut crate::app::FootprintEditorState, ctx: &ToolClickCtx) {
    ...
    let Some((line_id, t)) = pick else {
        editor.state.solve_warnings.push(
            "Break Track: click missed any Line — try clicking closer to a line stroke".into(),
        );
        editor.state.tool_pending = ToolPending::Idle;
        return;
    };
    ...
    match outcome {
        Some(Ok(result)) => {
            ...
            if !result.dropped_constraints.is_empty() {
                editor.state.solve_warnings.push(
                    "Break Track: a Midpoint constraint on the split Line was dropped".into(),
                );
            }
        }
        Some(Err(e)) => {
