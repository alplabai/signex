...
pub(super) fn apply(
    ...
    match tool {
        ...
        SketchTool::EdgeArc => edge_arc(editor, ctx),
        ...
        ToolPending::EdgeArcEnd { start, end } => {
            ...
            match circumcircle(
                ...
                Some((cx, cy, _radius)) => {
                    ...
                    // circle, the (start, mid, end) triangle's winding
                    // matches their cyclic order around it — CCW winding
                    // means walking CCW from `start` reaches `mid`
                    // before `end`.
                    let sweep_ccw = match orient2d(start_pos.into(), mid_pos.into(), end_pos.into())
                    {
                        Sign::Negative => false,
