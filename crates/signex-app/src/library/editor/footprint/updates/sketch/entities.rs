...
pub(in crate::library::editor::footprint::updates) fn apply(
    ...
    match msg {
        FootprintEditorMsg::SketchPlacePoint { x_mm, y_mm } => place_point(editor, x_mm, y_mm),
        FootprintEditorMsg::SketchMovePoint { id, dx, dy } => move_point(editor, id, dx, dy),
        FootprintEditorMsg::SketchMoveLine { id, dx, dy } => move_line(editor, id, dx, dy),
        FootprintEditorMsg::SketchResizeRoundPad {
            pad_idx,
            diameter_mm,
        } => resize_round_pad(editor, pad_idx, diameter_mm),
        _ => unreachable!(
...
fn move_line(
    ...
    let intersect =
        |p1: (f64, f64), d1: (f64, f64), p2: (f64, f64), d2: (f64, f64)| -> Option<(f64, f64)> {
    ...
    // v0.27 — propagate the edge drag to the literal pad bbox — see
    // `propagate_line_drag_to_pad_bboxes`.
    propagate_line_drag_to_pad_bboxes(editor, pre_drag_endpoints, dx, dy);
    editor.with_parts(|state, primitive| {
        CanvasState::sync_pads_to_primitive(state, primitive);
    });
    editor.canvas_cache.clear();
...
fn propagate_line_drag_to_pad_bboxes(
    ...
    let Some((world_start, world_end)) = pre_drag_endpoints else {
        return;
    };
    ...
    for pad_idx in 0..pad_count {
        ...
        let new_centre = {
            let pad = &editor.state.pads[pad_idx];
            pad.local_to_world_mm((new_xmin + new_xmax) / 2.0, (new_ymin + new_ymax) / 2.0)
        };
