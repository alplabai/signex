...
pub(super) fn apply(editor: &mut crate::app::FootprintEditorState, msg: FootprintEditorMsg) {
    match msg {
        FootprintEditorMsg::ToggleActiveBarMenu(menu) => toggle_active_bar_menu(editor, menu),
        FootprintEditorMsg::CloseActiveBarMenu => close_active_bar_menu(editor),
        FootprintEditorMsg::ActiveBarStub(label) => active_bar_stub(editor, label),
        FootprintEditorMsg::ApplyFilterPreset(idx) => apply_filter_preset(editor, idx),
        FootprintEditorMsg::ToggleAllFilters => toggle_all_filters(editor),
        FootprintEditorMsg::CaptureFilterPreset => capture_filter_preset(editor),
        FootprintEditorMsg::ActiveBarToggleSnap(flag) => active_bar_toggle_snap(editor, flag),
        FootprintEditorMsg::ActiveBarSetSnappingMode(mode) => {
            active_bar_set_snapping_mode(editor, mode)
        }
        FootprintEditorMsg::ActiveBarSetSnapSubTab(sub) => active_bar_set_snap_sub_tab(editor, sub),
        FootprintEditorMsg::ActiveBarRotateSelection => active_bar_rotate_selection(editor),
        FootprintEditorMsg::ActiveBarFlipSelection => active_bar_flip_selection(editor),
        FootprintEditorMsg::ActiveBarNudgeSelection => active_bar_nudge_selection(editor),
        FootprintEditorMsg::MoveByOpen => move_by_open(editor),
        FootprintEditorMsg::MoveBySetX(v) => move_by_set_x(editor, v),
        FootprintEditorMsg::MoveBySetY(v) => move_by_set_y(editor, v),
        FootprintEditorMsg::MoveByConfirm => move_by_confirm(editor),
        FootprintEditorMsg::MoveByCancel => move_by_cancel(editor),
        FootprintEditorMsg::AlignOpen => align_open(editor),
        FootprintEditorMsg::AlignSetHorizontal(op) => align_set_horizontal(editor, op),
        FootprintEditorMsg::AlignSetVertical(op) => align_set_vertical(editor, op),
        FootprintEditorMsg::AlignConfirm => align_confirm(editor),
        FootprintEditorMsg::AlignCancel => align_cancel(editor),
        FootprintEditorMsg::ActiveBarAlignSelectionToGrid => {
            active_bar_align_selection_to_grid(editor)
        }
        FootprintEditorMsg::ActiveBarMoveOriginToGrid => active_bar_move_origin_to_grid(editor),
        FootprintEditorMsg::ActiveBarSelectAll => active_bar_select_all(editor),
        FootprintEditorMsg::ActiveBarClearSelection => active_bar_clear_selection(editor),
        FootprintEditorMsg::ActiveBarSetSketchTool(tool) => {
            active_bar_set_sketch_tool(editor, tool)
        }
...
fn close_active_bar_menu(editor: &mut crate::app::FootprintEditorState) {
    editor.state.active_bar_menu = None;
}

fn active_bar_stub(editor: &mut crate::app::FootprintEditorState, label: &'static str) {
...
fn toggle_all_filters(editor: &mut crate::app::FootprintEditorState) {
...
fn capture_filter_preset(editor: &mut crate::app::FootprintEditorState) {
    ...
    if presets.len() < crate::active_bar::CUSTOM_FILTER_PRESET_LIMIT {
        ...
        let preset =
            crate::library::editor::footprint::filter_presets::capture_preset(&editor.state, name);
        presets.push(preset);
...
fn active_bar_toggle_snap(
    ...
    match flag {
        ...
        SnapOptionFlag::HorizontalVertical => opts.horizontal_vertical = !opts.horizontal_vertical,
        SnapOptionFlag::Angle => opts.angle = !opts.angle,
        SnapOptionFlag::Grid => opts.grid = !opts.grid,
        SnapOptionFlag::TrackVertices => opts.snap_track_vertices = !opts.snap_track_vertices,
...
fn active_bar_set_snapping_mode(
    editor: &mut crate::app::FootprintEditorState,
...
fn active_bar_set_snap_sub_tab(
    editor: &mut crate::app::FootprintEditorState,
    sub: crate::library::editor::footprint::state::SnapSubTab,
) {
...
fn active_bar_nudge_selection(editor: &mut crate::app::FootprintEditorState) {
...
fn move_by_open(editor: &mut crate::app::FootprintEditorState) {
    editor.state.move_by_modal = Some(Default::default());
    editor.state.active_bar_menu = None;
}

fn move_by_set_x(editor: &mut crate::app::FootprintEditorState, v: String) {
    if let Some(m) = editor.state.move_by_modal.as_mut() {
        m.dx_buf = v;
    }
}

fn move_by_set_y(editor: &mut crate::app::FootprintEditorState, v: String) {
    if let Some(m) = editor.state.move_by_modal.as_mut() {
        m.dy_buf = v;
    }
}
...
fn move_by_cancel(editor: &mut crate::app::FootprintEditorState) {
    editor.state.move_by_modal = None;
}
...
fn align_set_horizontal(
    editor: &mut crate::app::FootprintEditorState,
    op: Option<crate::library::editor::footprint::state::AlignOp>,
) {
...
fn align_cancel(editor: &mut crate::app::FootprintEditorState) {
    editor.state.align_modal = None;
}

fn active_bar_align_selection_to_grid(editor: &mut crate::app::FootprintEditorState) {
    // #146 + #433 — gate history + dirty on a real selection (see
...
fn active_bar_select_all(editor: &mut crate::app::FootprintEditorState) {
...
fn active_bar_clear_selection(editor: &mut crate::app::FootprintEditorState) {
