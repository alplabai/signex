...
pub(in crate::app::view) const MODAL_CLOSE_X_HOVER: Color = Color::from_rgba(0.78, 0.22, 0.22, 1.0);
    ...
    pub(super) fn preview_project_annotations(&self) -> Vec<AnnotatePreviewEntry> {
        ...
        if let Some(loaded) = self.document_state.active_loaded_project() {
            ...
            for sheet_entry in &loaded.data.sheets {
                ...
                if let Ok(parsed) = standard_parser::parse_schematic_file(&file_path) {
                    let title = sheet_entry.name.trim_end_matches(".standard_sch").to_string();
