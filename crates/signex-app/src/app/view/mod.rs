...
impl Signex {
    pub fn view(&self, window_id: iced::window::Id) -> Element<'_, Message> {
        ...
        if let Some(kind) = self.ui_state.windows.get(&window_id) {
            return match kind {
                super::states::WindowKind::DetachedModal(modal) => self.view_detached_modal(*modal),
                ...
                super::states::WindowKind::UndockedTab { .. } => self.view_main_for(window_id),
                super::states::WindowKind::DetachedPanel(kind) => {
