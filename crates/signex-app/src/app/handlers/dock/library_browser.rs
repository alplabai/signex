...
impl Signex {
    ...
    fn load_library_browser_state(&mut self, selected_library: String) -> Result<()> {
        let Some(library_root) = self.document_state.standard_lib_dir.clone() else {
            crate::diagnostics::log_warning("Library selection ignored because no Standard library directory is configured");
            return Ok(());
        };
    ...
    fn append_library_symbols(
        ...
        let parsed_library = standard_parser::parse_symbol_lib(&content)
