...
impl Signex {
    ...
    pub(crate) fn dispatch_transmission_line_calculator_message(
        ...
        match message {
            ...
            SmithChartMessage::CopyUrl => {
                match self
                    .ui_state
                    .transmission_line_calculator
                    .generated_online_smith_chart_url()
                {
                    Ok(url) => {
                        self.ui_state.transmission_line_calculator.file_status =
                            Some(format!("Copied URL to clipboard: {url}"));
                        iced::clipboard::write(url)
                    }
                    Err(err) => {
                        self.ui_state.transmission_line_calculator.url_error = Some(err);
                        Task::none()
                    }
                }
            }
            SmithChartMessage::OpenReferenceLink(link) => {
                self.ui_state.transmission_line_calculator.file_status =
                    Some(format!("Opening reference link: {}", link.url()));
                Task::perform(
                    async move {
                        webbrowser::open(link.url()).map_err(|err| {
                            format!("Failed to open reference link {}: {err}", link.url())
                        })
                    },
                    |result| {
                        Message::TransmissionLineCalculator(SmithChartMessage::ReferenceLinkOpened(
                            result,
                        ))
                    },
                )
