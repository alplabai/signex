...
impl Signex {
    pub fn new() -> (Self, Task<Message>) {
        ...
        let standard_lib_dir = find_standard_symbols_dir();
        let standard_libraries = standard_lib_dir
            .as_deref()
            .map(list_standard_libraries)
            .unwrap_or_default();
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match message {
                    ...
                    crate::dock::DockMessage::Panel(crate::panels::PanelMsg::SelectLibrary(
                        ...
                        if let Some(dir) = &self.standard_lib_dir {
                            ...
                            match std::fs::read_to_string(&path) {
                                Ok(content) => match standard_parser::parse_symbol_lib(&content) {
                                    Ok(symbols) => {
                        ...
                        match standard_parser::parse_schematic_file(&path) {
                            Ok(sheet) => {
                                ...
                                if let Some(dir) = path.parent() {
                                    ...
                                    if pro_path.exists()
                                        && let Ok(proj) = standard_parser::parse_project(&pro_path)
                                    {
            ...
            Message::SaveFile => {
                ...
                {
                    let content = standard_writer::write_schematic(sheet);
