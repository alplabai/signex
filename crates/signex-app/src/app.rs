...
fn find_standard_symbols_dir() -> Option<PathBuf> {
...
fn list_standard_libraries(dir: &std::path::Path) -> Vec<String> {
    std::fs::read_dir(dir)
        .ok()
        .map(|entries| {
...
impl Signex {
    pub fn new() -> (Self, Task<Message>) {
        ...
        let app = Self {
            ...
            panel_ctx: crate::panels::PanelContext {
                ...
                grid_size_mm: 2.54,
                properties_tab: 0,
                standard_libraries: find_standard_symbols_dir()
                    .map(|d| list_standard_libraries(&d))
                    .unwrap_or_default(),
                active_library: None,
                library_symbols: vec![],
            ...
            bottom_height: 120.0,
            active_menu: None,
            standard_lib_dir: find_standard_symbols_dir(),
            ...
            Message::Dock(msg) => {
                ...
                match &msg {
                    ...
                    crate::dock::DockMessage::Panel(
                        ...
                        let name = name.clone();
                        if let Some(dir) = &self.standard_lib_dir {
                            let path = dir.join(format!("{name}.standard_sym"));
                            match std::fs::read_to_string(&path) {
                                Ok(content) => {
                                    match standard_parser::parse_symbol_lib(&content) {
                                        Ok(symbols) => {
                            ...
                            get_node(&self.panel_ctx.project_tree, path)
                        {
                            if matches!(
                                    ...
                                    .and_then(|p| p.parent())
                                {
                                    ...
                                    if file_path.exists()
                                        && filename.ends_with(".standard_sch")
                                    {
                                        ...
                                        if let Some(idx) = self.tabs.iter().position(|t| t.path == file_path) {
                                            ...
                                            // Open new tab
                                            match standard_parser::parse_schematic_file(&file_path) {
                                                Ok(sheet) => {
                                                    self.tabs.push(TabInfo {
                                                        title: filename.replace(".standard_sch", ""),
                                                        path: file_path,
            ...
            Message::FileOpened(Some(path)) => {
                ...
                match ext {
                    "standard_pro" => {
                        ...
                        match standard_parser::parse_project(&path) {
                            Ok(proj) => {
                                ...
                                if let Some(root_sch) = &proj.schematic_root {
                                    ...
                                    match standard_parser::parse_schematic_file(&sch_path) {
                                        Ok(sheet) => {
                                            let title = root_sch.replace(".standard_sch", "");
                                            self.tabs.push(TabInfo {
                                                title,
                                                path: sch_path.clone(),
                    ...
                    "standard_sch" => {
                        // Direct schematic open — also try to find the .standard_pro
                        match standard_parser::parse_schematic_file(&path) {
                            Ok(sheet) => {
                                self.project_path = Some(path.clone());
                                // Try to find and parse the .standard_pro in the same directory
                                if let Some(dir) = path.parent() {
                                    let stem = path.file_stem().and_then(|s| s.to_str()).unwrap_or("");
                                    let pro_path = dir.join(format!("{stem}.standard_pro"));
                                    if pro_path.exists() {
                                        if let Ok(proj) = standard_parser::parse_project(&pro_path) {
            ...
            MenuMessage::OpenProject => Task::perform(
                async {
                    rfd::AsyncFileDialog::new()
                        .set_title("Open Standard Schematic")
    ...
    fn refresh_panel_ctx(&mut self) {
        ...
        self.panel_ctx = crate::panels::PanelContext {
            ...
            standard_libraries: self.panel_ctx.standard_libraries.clone(),
