                    ...
                    crate::dock::DockMessage::Panel(crate::panels::PanelMsg::SelectLibrary(
                        ...
                        if let Some(dir) = &self.standard_lib_dir {
                            ...
                            for library_name in libraries {
                                let path = dir.join(format!("{library_name}.standard_sym"));
                            ...
                            && matches!(node.icon, TreeIcon::Schematic | TreeIcon::Pcb)
                        {
                            ...
                            if let Some(dir) = self.project_path.as_ref().and_then(|p| p.parent()) {
                                ...
                                if file_path.exists() {
                                    ...
                                    } else if filename.ends_with(".standard_sch")
                                        ...
                                        match standard_parser::parse_schematic_file(&file_path) {
                                            Ok(sheet) => {
                                                self.open_schematic_tab(
                                                    file_path,
                                                    filename.replace(".standard_sch", ""),
                                                    sheet,
                                                );
                                            }
                                            Err(e) => eprintln!("Failed to parse {filename}: {e}"),
                                        }
                                    } else if filename.ends_with(".standard_pcb")
                                        || filename.ends_with(".snxpcb")
                                    {
                                        match standard_parser::parse_pcb_file(&file_path) {
                                            Ok(board) => {
                                                let title = filename
