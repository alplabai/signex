//! QA harness — exercise every v0.8 exporter against a real KiCad project
//! and report sizes / sheet counts / validation issues.
//!
//! Usage: `cargo run --example qa_harness -p signex-output -- <project.kicad_pro> [out_dir]`
//!
...
fn main() {
    ...
    let project_path = match args.next() {
        ...
        None => {
            eprintln!("usage: qa_harness <project.kicad_pro> [out_dir]");
            std::process::exit(2);
        }
    };
    let out_dir =
    ...
    let project = kicad_parser::parse_project(&project_path).expect("parse project");
    ...
    for (i, entry) in project.sheets.iter().enumerate() {
        ...
        let parsed = match kicad_parser::parse_schematic_file(&abs) {
            Ok(s) => s,
            Err(e) => {
