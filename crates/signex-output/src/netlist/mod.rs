//! Standard S-expression `.net` netlist export.
//!
//! See `OUTPUT_PLAN.md` §7. Emits byte-compatible Standard netlist format so
//! downstream tools (Standard CvPcb, kinet2pcb, legacy CAM) don't know Signex
//! was the emitter.
//!
...
mod standard_sexpr;
use standard_sexpr::{emit_comp, emit_header, emit_net};
...
fn build_net_graph_multi_sheet(
    ...
    let mut graph = standard_sexpr::build_net_graph(first_sheet, symbols);
...
impl Exporter for NetlistExporter {
    ...
    fn export(
        ...
        // Unnamed roots get `Net-(Rx-Padn)` auto-names per Standard convention.
        ...
        // Standard netlists use forward-slash separators in (source ...)
        ...
        let mut comp_section = vec![standard_parser::sexpr_builder::raw("components")];
        comp_section.extend(components);
        root_items.push(standard_parser::sexpr_builder::list(comp_section));

        // Add nets section
        let mut nets_section = vec![standard_parser::sexpr_builder::raw("nets")];
        nets_section.extend(nets_sexpr);
        root_items.push(standard_parser::sexpr_builder::list(nets_section));

        let root = standard_parser::sexpr_builder::list(root_items);
