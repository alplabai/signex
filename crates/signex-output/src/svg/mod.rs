        ...
        for sym in &sheet.schematic.symbols {
                ...
                && !ref_text.hidden
            {
                elements.push(SvgElement::Text {
                    ...
                    text: normalize_standard_text(&sym.reference),
                });
            }
                ...
                && !val_text.hidden
            {
                elements.push(SvgElement::Text {
                    ...
                    text: normalize_standard_text(&sym.value),
                });
            }
        }
    ...
    for lp in &lib.pins {
        ...
        if lib.show_pin_names && pin.name_visible && !pin.name.is_empty() && pin.name != "~" {
            let name_pos = if lib.pin_name_offset.abs() < 0.01 {
                // Standard offset=0 mode: put name perpendicular to tip.
                let perp = (-wdy, wdx);
            ...
            out.push(SvgElement::Text {
                ...
                text: normalize_standard_text(&pin.name),
            });
        }
        ...
        if lib.show_pin_numbers && pin.number_visible && !pin.number.is_empty() {
            ...
            out.push(SvgElement::Text {
                ...
                rotation_deg: 0.0,
                fill_rgb: (0.1, 0.1, 0.1),
                text: normalize_standard_text(&pin.number),
            });
        }
    }
}
