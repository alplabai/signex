...
//!
//! Format mirrors Standard's `.standard_*` style — flat S-expression tree —
//! so we can reuse `standard-parser::sexpr` for tokenisation and tree
...
use standard_parser::sexpr::{SExpr, parse as parse_sexpr};
...
/// Format an f64 with the minimum decimals that round-trips. Standard
