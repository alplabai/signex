...
fn parse_text_prop(prop_node: &SExpr, _fallback_pos: Point) -> TextProp {
    ...
    // Standard property texts (Reference/Value/custom fields) default to centered
...
/// - `(hide yes)`  — Standard 7+ list form with explicit "yes"
/// - `(hide)`      — Standard 6 list form with no argument
/// - `hide`        — Standard 5/6 bare-atom form inside `(effects ...)`
                ...
                Some("bezier") => {
                    ...
                    if let Some(pts) = child.find("pts") {
                        let points: Vec<Point> = pts
                            ...
                            .collect();
                        // Standard always has exactly 4 control points for a cubic bezier
                        if points.len() == 4 {
                            graphics.push(LibGraphic {
                                unit,
                                body_style,
...
fn parse_child_sheet(s: &SExpr) -> ChildSheet {
    ...
    ChildSheet {
        uuid: parse_uuid(s),
        // Modern Standard: "Sheet name" / "Sheet file" (with space).
...
/// Parse a `.standard_sch` file from a file path.
...
pub fn parse_project(path: &Path) -> Result<ProjectData, ParseError> {
    ...
    // libraries are recorded by `Project::libraries`.
    Ok(ProjectData {
        name: project_name,
...
fn collect_sheets(
    ...
    while let Some((filename, depth)) = queue.pop() {
        ...
        for line in content.lines() {
            ...
            // Only count top-level elements (depth == 2 because root standard_sch is depth 1)
        ...
        let name = if sheets.is_empty() {
            ...
            filename.trim_end_matches(".standard_sch").to_string()
        };
...
mod tests {
    ...
    fn minimal_standard_sch() -> String {
        r#"(standard_sch
  (version 20231120)
  (generator "test")
  ...
  (text "Hello World"
    ...
    #[test]
    fn parse_standard10_fields_default() {
        // Standard 10 fields should default correctly when absent
        let content = r#"(standard_sch
  (version 20260326)
  (generator "eeschema")
  ...
  (symbol "W5500_1_1")
    ...
    (sheet
        ...
        (property "Sheet file" "child.standard_sch" (at 20 41 0) (effects (font (size 1.27 1.27))))
        (instances
            (project "GateMagic"
