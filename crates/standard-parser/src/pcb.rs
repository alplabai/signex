...
/// Parse a `.standard_pcb` file from its string contents.
pub fn parse_pcb(content: &str) -> Result<PcbBoard, ParseError> {
    let root = sexpr::parse(content)?;

    if root.keyword() != Some("standard_pcb") {
        return Err(ParseError::InvalidSExpr("Not a Standard PCB file".to_string()));
    }
    ...
    let zones: Vec<Zone> = root
        ...
        .map(|z| {
            ...
            // Thermal -- under connect_pads node in Standard format
...
/// Parse a footprint from an S-expression node (reusable for both PCB and standalone .standard_mod)
...
/// Parse a `.standard_pcb` file from a file path.
pub fn parse_pcb_file(path: &Path) -> Result<PcbBoard, ParseError> {
    let content = std::fs::read_to_string(path)?;
    parse_pcb(&content)
}

/// Parse a standalone `.standard_mod` footprint file.
    ...
    if root.keyword() != Some("footprint") {
        ...
        if root.keyword() != Some("module") {
            return Err(ParseError::InvalidSExpr(
                "Not a Standard footprint file".to_string(),
            ));
        }
    }
