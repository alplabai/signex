...
/// Parse a `.standard_sym` symbol library file and return a map of symbol ID to `LibSymbol`.
pub fn parse_symbol_lib(input: &str) -> Result<HashMap<String, LibSymbol>, ParseError> {
    let root = sexpr::parse(input)?;

    if root.keyword() != Some("standard_symbol_lib") {
        return Err(ParseError::InvalidSExpr(
            "Not a Standard symbol library file".to_string(),
        ));
    }
    ...
    for sym_node in root.find_all("symbol") {
        ...
        // a top-level symbol that already exists in the parent standard_symbol_lib.
        if id.contains('_') {
...
mod tests {
    ...
    fn parse_minimal_symbol_lib() {
        let input = r#"(standard_symbol_lib
  (version 20231120)
  (generator "test")
  (symbol "Device:R"
    (pin_names (offset 0))
