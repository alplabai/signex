/// S-expression tokenizer and tree parser for Standard files.
/// Standard uses a Lisp-like format: (keyword arg1 "string arg" (nested ...))
...
mod tests {
    ...
    #[test]
    fn test_real_standard_file() {
        // Use STANDARD_TEST_FILE env var to point to a real .standard_sch for integration testing
        let path = match std::env::var("STANDARD_TEST_FILE") {
            Ok(p) => p,
        ...
        assert_eq!(expr.keyword(), Some("standard_sch"));
    }
