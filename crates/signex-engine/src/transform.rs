        ...
        match item.kind {
            SelectedKind::Symbol => self
                ...
                .map(|symbol| {
                    ...
                    // Field autoplace is reintroduced in v0.12 cleanroom rewrite
                    // (Wave 2.5). Until then, rotation preserves stored field
                    // positions; users can drag fields manually.
                    true
                })
                .unwrap_or(false),
            SelectedKind::SymbolRefField => self
                ...
                .map(|symbol| {
                    if let Some(ref mut ref_text) = symbol.ref_text {
                        ...
                        // Manual field rotation clears the autoplaced flag so
                        // a future autoplace pass will leave this field alone.
        ...
        match item.kind {
            SelectedKind::Symbol => self
                ...
                .map(|symbol| {
                    ...
                    // Field autoplace is reintroduced in v0.12 cleanroom rewrite
                    // (Wave 2.5). Until then, mirror preserves stored field
                    // positions.
                    true
                })
                .unwrap_or(false),
...
// ---------------------------------------------------------------------------
// Field autoplace — DELETED on `feature/v0.12-cleanroom-rewrite`.
//
// Both `autoplace_fields` and `autoplace_all_marked_fields` were a
// parameter-for-parameter mirror of an external tool's algorithm. They are
// reintroduced from a Signex-only spec in Wave 2.5 (see
// `docs/internal/CLEANROOM_REWRITE_PLAN.md` and
// `docs/RENDERING_RULES.md`). Until Wave 2.5 lands, rotate / mirror
// preserve stored field positions; this is intentional.
// ---------------------------------------------------------------------------
