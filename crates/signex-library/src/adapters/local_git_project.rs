...
impl LocalGitProjectAdapter {
    ...
    ///
    /// Existing `.gitattributes` is overwritten. The file is staged
    /// + committed by the migration flow's initial-commit step;
    /// callers don't need a separate commit.
    pub fn write_gitattributes(&self, use_lfs: bool) -> Result<(), LibraryError> {
        ...
        text.push_str("# Edit if you need to add custom rules; Signex won't overwrite manual edits\n");
        text.push_str("# unless you re-run \"Enable Version Control\".\n\n");
        text.push_str("# Native Signex formats are line-based UTF-8 — keep LF endings everywhere\n");
