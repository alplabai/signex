...
//!
//! Symbol/footprint diffs use [`kicad_parser::sexpr`] to walk the embedded
//! S-expression text and extract pins/pads. Pin identity is the pin number
//! (KiCad pin "name" is the printed label; "number" is the silk-stable
...
//! KiCad's `(at x y rot)` doesn't manufacture spurious "moved" entries.

use std::collections::{BTreeMap, BTreeSet};

use kicad_parser::sexpr::{self, SExpr};

use crate::component::Revision;
use crate::embed::{ParamMap, ParamValue, SupplierLink};
use crate::lifecycle::LifecycleState;

/// Position-equality epsilon (mm). KiCad coordinates round-trip in 1 µm.
...
/// KiCad pin nodes carry the silk-stable identifier as `(number "1")`.
