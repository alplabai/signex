...
impl DistributorAdapter for JlcpcbAdapter {
    ...
    fn lookup_by_mpn(&self, mpn: &str) -> Result<Vec<DistributorPart>, DistributorError> {
        if let Some(cache) = &self.cache
            && let Ok(Some(hit)) = cache.get(JLCPCB_PROVIDER_KEY, mpn, DEFAULT_TTL)
        {
            return Ok(vec![hit]);
        }
        ...
        if let (Some(cache), Some(first)) = (&self.cache, hits.first()) {
            ...
            if let Err(e) = cache.put(JLCPCB_PROVIDER_KEY, first) {
                tracing::warn!(
                    error = %e,
                    mpn = %mpn,
                    "JLCPCB cache write failed; live result returned but not persisted",
                );
            }
        }
        Ok(hits)
