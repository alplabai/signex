...
/// Expand KiCad `{name}` escape tokens to their literal characters.
pub fn expand_kicad_char_escapes(input: &str) -> String {
    if !input.contains('{') {
...
/// Evaluate a subset of KiCad/Altium-style expression variables.
///
/// Supported today:
/// - `${refdes:<key>}`
/// - `@{<name>}`
...
/// KiCad-style unnamed net fallback: `Net-(<refdes>-Pad<pin>)`.
///
/// Uses lexicographically smallest `(refdes, pin)` pair for deterministic
/// naming, matching the netlist side policy.
pub fn kicad_auto_net_name_from_pins(pins: &[(String, String)]) -> Option<String> {
    pins.iter()
...
pub fn parse_markup(input: &str) -> Vec<RichSegment> {
    ...
    while i < len {
        match bytes[i] {
            ...
            // Overbar alternative: ~(...) (KiCad syntax help allows this form).
            b'~' if i + 1 < len && bytes[i + 1] == b'(' => {
                if !normal_buf.is_empty() {
                    segments.push(RichSegment::Normal(std::mem::take(&mut normal_buf)));
                }
...
mod tests {
    ...
    #[test]
    fn expands_kicad_char_escapes() {
        assert_eq!(expand_kicad_char_escapes("A{slash}B{dollar}C"), "A/B$C");
    }
    ...
    #[test]
    fn kicad_auto_net_name_uses_lowest_pair() {
        let pins = vec![
        ...
        assert_eq!(
            kicad_auto_net_name_from_pins(&pins),
