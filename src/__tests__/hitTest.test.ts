...
describe("hitTest", () => {
  it("hit on wire segment returns wire type", () => {
    ...
    // Wire w1 goes from (10,16.19) to (20,16.19) — click at midpoint
    const hit = hitTest(data, 15, 16.19);
  ...
  it("hit on wire endpoint returns wireEndpoint type", () => {
    const data = createSimpleSchematic();
    // Wire w1 start at (10, 16.19)
    const hit = hitTest(data, 10, 16.19);
    expect(hit).not.toBeNull();
  ...
  it("hit on label returns label type", () => {
    const data = createSimpleSchematic();
    const hit = hitTest(data, 20, 16.19);
    expect(hit).not.toBeNull();
  ...
  it("hit on no-connect returns noConnect type", () => {
    const data = createSimpleSchematic();
    const hit = hitTest(data, 10, 23.81);
    expect(hit).not.toBeNull();
  ...
  it("priority: junction before wire", () => {
    const data = createSimpleSchematic();
    data.junctions.push({ uuid: "j-test", position: { x: 15, y: 16.19 } });
    const hit = hitTest(data, 15, 16.19);
    expect(hit).not.toBeNull();
...
describe("boxSelect", () => {
  ...
  it("crossing mode (right-to-left): selects wires that cross box", () => {
    const data = createSimpleSchematic();
    // Right-to-left box that crosses wire w1 (10→20, y=16.19)
